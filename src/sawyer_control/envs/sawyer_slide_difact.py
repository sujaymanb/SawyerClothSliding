...
class SawyerSlideEnv(SawyerEnvBase):
	...
	def __init__(self,
		...
		# for high
		#self.start_pose = (np.array([-0.2852793 ,  0.18365528, -1.77685845,  1.0974561 , -1.22174418,-1.57684469, -4.33910179]), np.array([ 0.51826346, -0.35206413,  0.29639095]))
	...
	def _set_action_space(self):
		...
		if self.inc_angle and not self.fix_hor:
	...
	def _act(self, action):
		...
		elif self.inc_angle and self.fix_hor:
			self._position_act(np.array([0., self.position_action_scale[0], action[0]*self.position_action_scale[1], action[1]*self.angle_scale]))
		else:
	...
	def _position_act(self, action):
		...
		current_o = pose[3:]
		orientation = self.orient_action(current_o,action[3],1)
	...
	def _reset_robot(self):
		if not self.reset_free:
			...
			for _ in range(2):
				...
				# for high
				#self.set_env_state((np.array([-0.27931055,  0.21668945, -2.10898352,  1.19692969, -1.17321193,-1.71543062, -4.47294903]), np.array([ 0.52201331, -0.29279199,  0.39882207])))
			for _ in range(2):
